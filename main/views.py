...
def apiCoursesAddHandler(request):
    if request.POST:
        ...
        title = request.POST.get('title', '')
        status = int(request.POST.get('status', 0))
        course_type = int(request.POST.get('course_type', 0))
        ...
        info2_text = request.POST.get('info2_text', '')
        info3 = request.POST.get('info3', '')
        info3_text = request.POST.get('info3_text', '')
        info4 = request.POST.get('info4', '')
        info4_text = request.POST.get('info4_text', '')
        lessons_count = int(request.POST.get('lessons_count', 0))
        description = request.POST.get('description', '')
        is_free = request.POST.get('is_free', '')
        is_profession = request.POST.get('is_profession', '')
        is_certified = request.POST.get('is_certified', '')
        logo = request.POST.get('logo', '')
        if action == 'add':
            cc = Course()
            cc.title = title
            cc.status = status
            cc.course_type = course_type
            cc.category_id = category_id
            cc.lessons_count = lessons_count
            cc.info1 = info1
            cc.info1_text = info1_text
            cc.info2 = info2
            cc.info2_text = info2_text
            cc.info3 = info3
            cc.info3_text = info3_text
            cc.info4 = info4
            cc.logo = logo
            cc.info4_text = info4_text
            ...
            if is_profession:
                cc.is_profession = True
            else:
                cc.is_profession = False
            if is_certified:
                cc.is_certified = True
            else:
                cc.is_certified = False
            #cc.category_id = CourseCategory.objects.get(id=category_id)
            cc.save()

    categories = CourseCategory.objects.all()

    return render(request, 'api/courses_add.html', {'categories': categories, })
